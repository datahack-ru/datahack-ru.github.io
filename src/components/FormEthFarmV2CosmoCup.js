...
import FarmStakingAbi from '../constants/abis/FarmStaking.json';
...
import { cosmoAddress, cupAddress, uniswapV2CosmoCupLpAddress } from '../constants';
import { getUniswapV2AddLiquidityLink, getUniswapV2RemoveLiquidityLink } from '../utils/uniswap';



const tokenAddress = uniswapV2CosmoCupLpAddress;

const farmAddress = '0x90940B9A36B11ca49e0Ac64e8579504Da13d2775'; // ?
const symbol = 'COSMO/CUP-LP';
const addLiquidityLink = getUniswapV2AddLiquidityLink(cosmoAddress, cupAddress);
const removeLiquidityLink = getUniswapV2RemoveLiquidityLink(cosmoAddress, cupAddress);



function FormEthFarmV2Cup(props) {
  ...
  const [farmRate, setFarmRate] = useState('499999999999');
  ...
  if (!farmContract) setFarmContract(new ethers.Contract(
    farmAddress, FarmStakingAbi, api.ethProvider
  ));
  ...
  const updateTotalStakedAmount = async () => {
    ...
    } catch (error) { console.error('updateTotalStakedAmount', error); }
  }
  ...
  const updateFarmRate = async () => {
    ...
    } catch (error) { console.error('updateFarmRate', error); }
  }
  ...
  const updateUnstakedAmount = async () => {
    ...
    } catch (error) { console.error('updateUnstakedAmount', error); }
  }
  ...
  const updateStakedAmount = async () => {
    ...
    } catch (error) { console.error('updateStakedAmount', error); }
  }
  ...
  const updatePendingReward = async () => {
    ...
    } catch (error) { console.error('updatePendingReward', error); }
  }
  ...
  const bonusPool = props.data ? props.data['farm2_0x1_cosmo_cup_lp'] : {};
  return (
    <Farm
