...
const FarmPanes = (props: any) => {
  ...
  return [
    ...
    {
      menuItem: <Button fluid className="farm__btn-2" onClick={() => null} >{t('Stake')}</Button>,
        ...
        <Table.Body>
          <Table.Row>
            ...
            <Table.Cell textAlign="right">
              <Number value={totalStakedAmount} decimalScale={0} fixedDecimalScale={true} suffix={` LP`} />
            </Table.Cell>
          </Table.Row>
          <Table.Row>
            <Table.Cell>{t('Farm rate')}</Table.Cell>
            <Table.Cell textAlign="right">
              <Number value={farmRate} decimalScale={0} fixedDecimalScale={true} suffix={` CCLP`} /> (<Number value={farmRate * cclpBscPrice} decimalScale={0} prefix={`$`} />)
            </Table.Cell>
          </Table.Row>
          <Table.Row className="farm__mark">
            <Table.Cell>{t('My stake')}</Table.Cell>
            <Table.Cell textAlign="right">
              <Number value={stakedAmount} decimalScale={0} fixedDecimalScale={true} suffix={` LP`} />
            </Table.Cell>
          </Table.Row>
          <Table.Row className="farm__mark">
            <Table.Cell>{t('My share')}</Table.Cell>
            <Table.Cell textAlign="right">
              <Number value={myFarmShare} decimalScale={4} suffix={`%`} />
            </Table.Cell>
          </Table.Row>
          <Table.Row className="farm__mark">
            <Table.Cell>{t('My rate')} / {t('day')}</Table.Cell>
            <Table.Cell textAlign="right">
              <Number value={myFarmRate} decimalScale={0} fixedDecimalScale={true} suffix={` CCLP`} /> (<Number value={myFarmRate * cclpBscPrice} decimalScale={4} prefix={`$`} />)
            </Table.Cell>
          </Table.Row>
          <Table.Row className="farm__mark">
            <Table.Cell>{t('Unclaimed rewards')}</Table.Cell>
            <Table.Cell textAlign="right">
              <Number value={pendingReward} decimalScale={0} fixedDecimalScale={true} suffix={` CCLP`} /> (<Number value={pendingReward * cclpBscPrice} decimalScale={4} prefix={`$`} />)
            </Table.Cell>
          </Table.Row>
          <Table.Row className="farm__mark-1">
            <Table.Cell>{t('Wallet Balance')}</Table.Cell>
            <Table.Cell textAlign="right">
              <Number value={unstakedAmount} decimalScale={0} fixedDecimalScale={true} suffix={` LP`} />
            </Table.Cell>
          </Table.Row>
        </Table.Body>
      </Table>
      ...
      */}
      ...
      <div className="farm__links">
        <Button
          ...
          href={addLiquidityLink} fluid
        >{t('Add liquidity')}</Button>
        <Button
          className="farm__btn-3"
          as='a' {...getExternalLinkProps()}
          href={removeLiquidityLink} fluid
