...
function PriceChart() {
  ...
  useEffect(() => {
    const chart = createChart('price-chart', chartStyles);
    const areaSeries = chart.addAreaSeries(chartLineSeriesStyles);
    ...
    chart.subscribeCrosshairMove((param) => {
      if (param.time) {
        ...
        setState({
          legend: 'CCLP' + ' $' + price.toFixed(2),
          time: param.time.year + '-' + param.time.month + '-' + param.time.day,
        });
      } else {
        setState({
          legend: 'CCLP',
  ...
  //style={{ color: 'white' }}
  return (
    <div id='price-chart-body' style={{ position: 'relative' }} >
      <div id='price-chart'></div>
      <div className='price-chart-legend-row1' style={{ color: 'black' }}>
        {state.legend}
      </div>
      <div className='price-chart-legend-row2' style={{ color: 'black' }}>
        {state.time}
      </div>
    </div>
  );
